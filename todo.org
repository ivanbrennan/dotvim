...
*** I initially tried ~set iskeyword+=!~, but this causes other problems, such as including a leading ! in tag lookups. Instead, a better workaround is:
#+begin_src vimscript
...
#+end_src
This makes use of the ~<Plug><cword>~ mapping that vim-ruby provides to correcly identify the Ruby cursor identifier.
See:
